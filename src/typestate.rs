...
#[flux_rs::opaque]
#[flux_rs::refined_by(enabled:bool, direction: bool, mode: int)]
struct GpioConfig {
...
impl GpioConfig {
    ...
    #[flux_rs::trusted]
    #[flux_rs::sig(fn(me: &strg GpioConfig, is_enabled: bool) ensures me: GpioConfig{v: v.enabled == is_enabled})]
    pub fn set_enable(&mut self, _is_enabled: bool) {
        // self.periph.modify(|_r, w| w.enable().set_bit(is_enabled));
    }

    #[flux_rs::trusted]
    #[flux_rs::sig(fn(me: &strg {GpioConfig[@old] | old.enabled}, is_output: bool) ensures me: GpioConfig[old.enabled, is_output, old.mode] )]
    pub fn set_direction(&mut self, _is_output: bool) {
        // self.periph.modify(|_r, w| w.direction().set_bit(is_output));
    }

    #[flux_rs::trusted]
    #[flux_rs::sig(fn(me: &strg {GpioConfig[@old] | old.enabled && !old.direction }, variant: InputMode) ensures me: GpioConfig[old.enabled, old.direction, variant] )]
    pub fn set_input_mode(&mut self, _variant: InputMode) {
        // self.periph.modify(|_r, w| w.input_mode().variant(variant));
    }

    #[flux_rs::trusted]
    #[flux_rs::sig(fn(me: &strg {GpioConfig[@old] | old.enabled && old.direction }, is_high: bool) ensures me: GpioConfig[old.enabled, old.direction, if is_high { 1 } else { 0 }] )]
    pub fn set_output_status(&mut self, _is_high: bool) {
        // self.periph.modify(|_r, w| w.output_mode.set_bit(is_high));
    }

    #[flux_rs::trusted]
    #[flux_rs::sig(fn(me: &GpioConfig{v: v.enabled && !v.direction }) -> bool)]
...
impl GpioConfig {
    ...
    #[flux_rs::sig(fn (me: &strg { GpioConfig[@old] | old.enabled} ) ensures me: GpioConfig[old.enabled, false, 2])]
