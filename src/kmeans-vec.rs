...
fn nearest(centers: &Vec<Point>, x: &Point) -> (usize, (Point, Weight)) {
    // let distances = centers.smap(x, |x, c| distance(x, &c));
    let distances = (0..centers.len())
        .map(|i| distance(x, &centers[i]))
        .collect::<Vec<f32>>();
...
pub fn kmeans(_n: usize, mut centers: Vec<Point>, points: Vec<Point>) {
    for _ in 0..100 {
        ...
        for (i, (x, w)) in weighted_centers {
            centers[i] = x.map(x, |xi| fdiv(*xi, w));
        }
    }
}
