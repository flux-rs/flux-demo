...
//     x.smap(&w, |w, xi| *xi / (*w as f32))
// }

#[flux::sig(fn (n: usize, centers: {RVec<RVec<f32>[n]>[@k] | 0 < k}, points: RVec<RVec<f32>[n]>))]
pub fn kmeans(_n: usize, mut centers: RVec<RVec<f32>>, points: RVec<RVec<f32>>) {
    for _ in 0..100 {
        ...
        for (i, (x, w)) in new_centers {
            centers.set(i, x.smap(&w, |w, xi| *xi / (*w as f32)));
        }
    }
}
