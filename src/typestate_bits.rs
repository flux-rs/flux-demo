...
// snapshot using actual bit-veckery
...
defs! {
    ...
    fn get_mode(modes: bitvec<32>, index: int) -> Mode {
        let val = (modes >> bv_int_to_bv32(index)) & 1;
        if val == 0 {
            Mode::Input
        } else {
            Mode::Output
...
// For simplicity in this demo, we'll just use factory functions to create GPIO ports
// In a real embedded system, these would be singleton instances

impl Gpio {
    // Factory functions to create GPIO port instances
    pub fn gpioa() -> Gpio {
        Gpio::new(0x4800_0000)
    }

    pub fn gpiob() -> Gpio {
        Gpio::new(0x4800_0400)
    }

    pub fn gpioc() -> Gpio {
        Gpio::new(0x4800_0800)
    }

    #[spec(fn(addr: usize) -> Gpio[0])]
    pub fn new(addr: usize) -> Gpio {
        // Create a new Gpio instance with all pins in input mode (mode bits = 0)
        Gpio {
            modes: ZERO, // All pins start in input mode (bit value 0)
            output: 0,   // Initialize output register to 0
            input: 0     // Initialize input register to 0
        }
    }
}
...
pub fn example_different_ports() {
    ...
    let mut gpio_a = Gpio::gpioa();
    let mut gpio_b = Gpio::gpiob();
    let mut gpio_c = Gpio::gpioc();
